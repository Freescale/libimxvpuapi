...
typedef struct
{
	...
	ImxVpuApiEncReturnCodes (*update_cyclic_intra_refresh_params)(void *h1_encoder);
}
...
struct _ImxVpuApiEncoder
{
	...
	/* The H1 encoder uses cyclic intra refresh. The start defines the
	 * macroblock index where the first intra macroblock shall be in
	 * the frame. The interval defines the distance between intra
	 * macroblocks. For example, start 2 interval 8 means that
	 * macroblock #2 will be the first intra macroblock in the frame.
	 * The second one will be #10, the third will be #18 etc.
	 * To calculate the interval out of the min_intra_refresh_mb_count
	 * parameter from ImxVpuApiEncOpenParams, this formula is used:
	 *
	 * cyclic_intra_refresh_interval = num_macroblocks_per_frame / min_intra_refresh_mb_count
	 *
	 * The idea here is that if for example the user wants one intra macroblock
	 * in the frame (so, min_intra_refresh_mb_count is 1), then the interval
	 * must be the total amount of macroblocks. If two intra macroblocks shall
	 * be present in the frame, then the interval must be half the amount of
	 * macroblocks etc. */
	 ...
	 * and is used for when cyclic intra refresh is active (see above). */
...
ImxVpuApiEncReturnCodes imx_vpu_api_enc_open(ImxVpuApiEncoder **encoder, ImxVpuApiEncOpenParams *open_params, ImxDmaBuffer *stream_buffer)
{
	...
	*encoder = malloc(sizeof(ImxVpuApiEncoder));
	...
	/* Calculate the number of macroblocks in a frame, rounding up to also
	 * include "half-macroblocks". This is needed for cyclic intra refresh. */
	(*encoder)->num_macroblocks_per_frame = ((fb_metrics->actual_frame_width + 15) / 16) * ((fb_metrics->actual_frame_height + 15) / 16);
	IMX_VPU_API_DEBUG("number of macroblocks per frame: %zu", (*encoder)->num_macroblocks_per_frame);
	...
	{
		(*encoder)->cyclic_intra_refresh_interval = 0;
		IMX_VPU_API_DEBUG("cyclic intra refresh interval is not used");
	}
...
ImxVpuApiEncReturnCodes imx_vpu_api_enc_encode(ImxVpuApiEncoder *encoder, size_t *encoded_frame_size, ImxVpuApiEncOutputCodes *output_code)
{
	...
	*encoded_frame_size = 0;
	...
	/* Perform the intra refresh cycling to ensure that all macroblocks are
	 * rotated through. The H1 encoder does not do this automatically. For
	 * example, with interval 4, the first frame macroblock are encoded like:
	 *
	 * I d d d I d d d I d d d I d d d ...
	 *
	 * (I = intra macroblock  d = delta macroblock)
	 *
	 * After shifting the cyclic intra refresh start here, the macroblocks
	 * in the second frame are encoded like this:
	 *
	 * d I d d d I d d d I d d d I d d ...
	 *
	 * And in the third frame:
	 *
	 * d d I d d d I d d d I d d d I d ...
	 *
	 * And in the fourth frame:
	 *
	 * d d d I d d d I d d d I d d d I ...
	 *
	 * At the fifth frame, the start is back to 0:
	 *
	 * I d d d I d d d I d d d I d d d ...
	 *
	 * etc. */
	if (encoder->use_intra_refresh)
	{
		switch (encoder->encoded_frame_type)
		{
			...
			case IMX_VPU_API_FRAME_TYPE_B:
			case IMX_VPU_API_FRAME_TYPE_BI:
				encoder->cyclic_intra_refresh_start = (encoder->cyclic_intra_refresh_start + 1) % encoder->cyclic_intra_refresh_interval;
				break;
			default:
				break;
		}

		encoder->h1_encoder_functions->update_cyclic_intra_refresh_params(encoder->h1_encoder);
		IMX_VPU_API_LOG("updated cyclic intra refresh: start %zu interval %zu", encoder->cyclic_intra_refresh_start, encoder->cyclic_intra_refresh_interval);
	}
...
static ImxVpuApiEncReturnCodes h1_vp8_update_cyclic_intra_refresh_params(void *h1_encoder);
...
static HantroH1EncoderFunctions const h1_vp8_encoder_functions = {
	...
	.update_cyclic_intra_refresh_params = h1_vp8_update_cyclic_intra_refresh_params,
};
...
static ImxVpuApiEncReturnCodes h1_vp8_open_encoder(ImxVpuApiEncoder *base, void **h1_encoder)
{
	...
	coding_control.cirStart = base->cyclic_intra_refresh_start;
	coding_control.cirInterval = base->cyclic_intra_refresh_interval;
	...
	IMX_VPU_API_DEBUG("  first cyclic intra refresh macroblock: %" PRIu32, (uint32_t)(coding_control.cirStart));
	IMX_VPU_API_DEBUG("  cyclic intra refresh macroblock interval: %" PRIu32, (uint32_t)(coding_control.cirInterval));
...
static ImxVpuApiEncReturnCodes h1_vp8_update_cyclic_intra_refresh_params(void *h1_encoder)
{
	...
	coding_control.cirStart = encoder->base->cyclic_intra_refresh_start;
	coding_control.cirInterval = encoder->base->cyclic_intra_refresh_interval;
...
static ImxVpuApiEncReturnCodes h1_h264_update_cyclic_intra_refresh_params(void *h1_encoder);
...
static HantroH1EncoderFunctions const h1_h264_encoder_functions = {
	...
	.update_cyclic_intra_refresh_params = h1_h264_update_cyclic_intra_refresh_params,
};
